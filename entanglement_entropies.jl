function von_Neumann_entropy(psi::MPS, b::Int; cutoff=1e-12)
    """
    Calculate the von Neumann entropy of the MPS `psi` biparted before site `b`.
    """
    (b <= 0 || b >= length(psi)) && return 0.0
    psi_tmp = orthogonalize(psi,b)
    llink = linkinds(psi_tmp,b-1)
    lsite = siteinds(psi_tmp,b)
    _ , S, _ = svd(psi_tmp[b], (llink..., lsite...); cutoff=cutoff)
    SvN = 0.0
    for k = 1:dim(S,1)
        p = S[k, k]*S[k, k]
        SvN -= p*log2(p)
...
function zeroth_entropy(psi::MPS, b::Int; cutoff=1e-12)
    """
    Calculate the zeroth order Renyi entropy of the MPS `psi` biparted before site `b`.
    """
    (b <= 0 || b >= length(psi)) && return 0.0
    psi_tmp = orthogonalize(psi,b)
    llink = linkinds(psi_tmp,b-1)
...
function Renyi_entropy(psi::MPS, b::Int, n::Real; cutoff=1e-12)
    """
    Calculate the n-th order Renyi entropy of the MPS `psi` biparted before site `b`.
    """
    (b <= 0 || b >= length(psi)) && return 0.0
