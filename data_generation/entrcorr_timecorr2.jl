...
@everywhere begin
    ...
    function entrcorr_average_wrapper(prob::Real, lsize::Int)
        ...
        avg = EntrCorrAverager{type}(b, 2lsize; n=1, op="Sz")
        # core calculation
        threshold = 5e-7 * (ttotal * lsize)
        maxbond = 20lsize
        timecorr, truncerr = timecorrelation!(psi, ttotal, 8lsize, mnt, ("Z", b, "Z", b), avg; 
            cutoff=cutoff, maxdim=maxbond, etol=threshold)
        
        psi = nothing  # free memory
        return avg, timecorr, truncerr
    end
end
...
let 
    ...
    h5open("data/nhcnot_entrcorr_avg_L$(L1)_$(dL)_$(L2)_$(nprob)x1.h5", "r+") do file
        ...
        dset5 = create_dataset(grp, "timecorrs", datatype(type), dataspace(4L2, nprob, nL))
        write(dset5, timecorrs)

        dset6 = create_dataset(grp, "truncerrs", datatype(type), dataspace(nprob, nL))
        write(dset6, truncerrs)
    end
end